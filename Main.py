...
def run(testParameters):
    ...
    model, auROC, auPRC, Fmax, mcc = tp.trainNetwork(testParameters.epochs)

    ### if called from the SingleTermWorkflow.py or ToxDL.py, get the full training set (train + valid) and test_set, for
    ...
    if sys.argv[0] == 'ToxDL.py':
        ...
        test_set = im.getSequences(testDatasetFile, testParameters.ngramsize, testParameters.maxLength)
        return model, full_train_set, test_set, auROC, auPRC, Fmax, mcc
    else:
        return model, auROC, auPRC, Fmax, mcc
